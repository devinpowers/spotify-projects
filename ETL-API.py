...
def spotify_func1(sp):
    ...
    # Postgres
    conn = psycopg2.connect(host = "localhost", user = "devinpowers",port="5433", dbname = "spotifynew")
    cur = conn.cursor()
    engine = create_engine('postgresql+psycopg2://devinpowers@localhost:5433/spotifynew') # this is 
...
def spotify_func2(sp):
    ...
    track_analysis_df.to_sql("tmp_spotify_audio_new", con = engine, if_exists='append', index = False)
